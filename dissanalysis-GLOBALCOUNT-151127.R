...
#count training organisations/trainers impacted at impact levels 4,7,8
levP<-subset(nodouble_g, LEVEL=="P")

#lev_highImpact<-subset(nodouble_g, IMPACT %in% c('4','6','7','8'))
#lev_HI_sum<-sum(lev_highImpact$IMPACTEDPEOPLE)

#levT_lowImpact<-subset(nodouble_g, IMPACT %in% c('1','2','3','5'))
#levT_LOW_sum<-sum(levT_lowImpact$IMPACTEDPEOPLE)

levS_highImpact<-subset(levS, IMPACT %in% c('4','6','7','8'))
levS_HI_sum<-sum(levS_highImpact$IMPACTEDPEOPLE)
